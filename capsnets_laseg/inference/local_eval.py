...
def pred_data_2D_per_sample(model, x_dir, y_dir, fnames, pad_shape=(256, 320),
                            ...
                            ct=False):
    """
    Loads raw data, preprocesses it, predicts 3D volumes slicewise (2D) one
    sample at a time, and pads to the original shape.
    Assumptions:
        The segmentation task you're working with is binary.
        The multi-output models output only have two outputs:
            (prediction mask, reconstruction mask)
        The .nii.gz files have the shape: (x, y, z) where z is the number of
        slices.
    Args:
        model: keras.models.Model instance
        x_dir: path to test images
        y_dir: path to the corresponding test masks
        fnames: files to evaluate in the directories; assuming that the input
            and labels are the same name
        ...
        mean_patient_shape: (z,x,y) representing the average shape.
            Defaults to (115, 320, 232).
        ct: whether or not the data is a CT scan or not. Defaults to False.
    Returns:
        actual_y: actual labels
        padded_pred: stacked, thresholded predictions (padded to original
            shape)
        padded_recon: stacked, properly padded reconstruction. Defaults to
            None if the model only outputs segmentations.
    ...
    for id in fnames:
        ...
        # preprocessing
        preprocessed_x, preprocessed_y, coords = isensee_preprocess(x, y,
                                                                    orig_spacing=None,
                                                                    get_coords=True,
                                                                    ct=ct,
        ...
        if isinstance(predicted, (list, tuple)):
            ...
            padded_recon = undo_reshape_and_nonint_extraction(reconstruction,
                                                              prior_reshape_shape=preprocessed_x.shape,
        ...
        padded_pred = undo_reshape_and_nonint_extraction(predicted,
                                                         prior_reshape_shape=preprocessed_x.shape,
                                                         orig_shape=orig_shape,
                                                         coords=coords,
                                                         pad_value=0)
...
def undo_reshape_and_nonint_extraction(pred, prior_reshape_shape, orig_shape,
                                       coords, pad_value=0):
    """
    Undoes the reshape padding and pads to compensate for `extract_nonint_region`.
    Args:
        pred: prediction from model in the form (z, x, y)
        prior_reshape_shape: the shape after `isensee_preprocess` and before
            it was reshaped to be fed into a model
        orig_shape: original shape of the raw Nifti1Image, with axes
            transposed to (z, x, y)
        coords: Coordinates from `extract_nonint_region`
        pad_value: Pad value for `pad_nonint_extraction`
    Returns:
        The padded prediction that corresponds to the original labels except
        with the axes transposed to (z, x, y)
    """
...
def evaluate_2D(y, pred):
    """
    Evaluates the generated predictions (binary only) with precision, recall
    and dice (f1-score)
    Args:
