...
class AdaptiveNetwork(object):
    ...
    * Primarily for making AdaptiveUNets recursively
    ...
    Attributes:
        ...
        base_pool_size: either a int or tuple/list that represents the pool
            size. The pool size is inferred from the int.
    """
    def __init__(self, input_shape, max_pools=5, starting_filters=30,
                 base_pool_size=2):
        ...
        if isinstance(self.base_pool_size, (tuple, list)):
            assert len(base_pool_size) == self.ndim, \
                "Make sure that base_pool_size matches the number of dimensions!"
    ...
    def _pool_statistics(self):
        ...
        Returns:
            a list of the total number of pools for each axes
            (excluding the channels dimension)
            ** Note: can be used to return a corresponding list of pooling
            sizes; i.e. (1,2,2)
        """
        ...
        # max possible shape after downsampling to the deepest depth
        max_down_shape = np.asarray([8 for i in range(self.ndim)],
                                    dtype=np.int32)
        axes_dict = {}
        n_pool = 0
...
#         if np.array_equal(axes_dict['min_shape'], max_down_shape):
    ...
    def generate_pool_sizes(self, n_pools):
        ...
        Args:
            ...
            base_pool_size: default pool size for MaxPooling and Upsampling
                layers
        """
        ...
        for (first, before) in zip(range(1, n_pools.size), range(0, n_pools.size - 1)):
            if not ordered[first] == ordered[before]: # They shouldn't be the same, or else you wouldn't need to add more pools
                ...
                new_size = self._get_single_pool_size(n_pools=n_pools,
                                                      threshold=ordered[before])
    ...
    def _get_single_pool_size(self, n_pools, threshold):
        """
        Handles cases where certain axes should not be pooled and returns an
        augmented pool size
        Args:
            n_pools:
    ...
    def _compute_patch_shape(median_patient_shape):
        """
        Computes the patch shape from the median patient shape and the
        ...
        * Need to also adjust batch size but this was not implemented
        ...
        if total_median_voxels <= 128**3:
            ...
            orig_aspect_ratio = np.divide(orig_patch_shape,
                                          np.asarray([138, 169, 138]))
            patch_shape = np.round(orig_aspect_ratio * median_patient_shape,
                                   decimals=0)
        return patch_shape
...
def context_module_2D(input_layer, n_filters, pool_size=(2,2), n_convs=2,
    ...
    """
    [2D]; Channels_last
    Context module (Downsampling compartment of the U-Net):
        `n_convs` Convs (w/ LeakyReLU and BN) -> MaxPooling
    Args:
        input_layer:
        n_filters:
        pool_size: if None, there will be no pooling (default: (2,2))
        n_convs: Number of convolutions in the module
        activation: the activation name; the only advanced activation
            supported is 'LeakyReLU' (default: 'LeakyReLU')
    Returns:
    ...
    for conv_idx in range(n_convs):
        if conv_idx == 0:
            if activation == 'LeakyReLU':
                ...
                act = Conv2D(n_filters, kernel_size=(3,3), padding='same',
...
def localization_module_2D(input_layer, skip_layer, n_filters,
                           upsampling_size=(2,2), n_convs=2,
                           activation='LeakyReLU', transposed_conv=False):
    """
    [2D]; Channels_last
    Localization module (Downsampling compartment of the U-Net):
        UpSampling3D -> `n_convs` Convs (w/ LeakyReLU and BN)
    Args:
        input_layer:
        ...
        activation: the activation name; the only advanced activation
            supported is 'LeakyReLU' (default: 'LeakyReLU')
        transposed_conv: boolean on whether you want transposed convs or
            UpSampling2D (default: False, which is UpSampling2D)
    Returns:
        keras layer after double convs w/ LeakyReLU and BN in-between
        upsampled output
    """
    if transposed_conv:
        upsamp = Conv2DTranspose(n_filters, (2, 2), strides=(2, 2),
                                 padding='same')(input_layer)
