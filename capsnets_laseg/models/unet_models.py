...
class AdaptiveUNet(model_utils.AdaptiveNetwork):
    """
    Isensee's 2D and 3D U-Nets for Heart Segmentation from the MSD that follows
    the conditions:
    ...
    Attributes:
        ...
        input_shape: The shape of the input including the number of input
            channels; (z, x, y, n_channels)
        ...
        starting_filters:
    """
    def __init__(self, n_convs, input_shape, n_classes=1, max_pools=5,
                 starting_filters=30):
        super().__init__(input_shape, max_pools, starting_filters,
                         base_pool_size=2)
        self.n_convs = n_convs
        self.n_classes = n_classes
        if self.ndim == 2:
            self.context_mod = partial(model_utils.context_module_2D,
                                       n_convs=n_convs)
            self.localization_mod = partial(model_utils.localization_module_2D,
                                            n_convs=n_convs)
    ...
    def _build_predictor(self, input_layer):
        """
        Takes a keras layer as input and returns the subsequent classifer
        ...
        * Activations: sigmoid for binary and softmax for multiclass
        ...
        if self.ndim == 2:
            if self.n_classes == 1:
                conv_seg = Conv2D(self.n_classes, kernel_size=kernel_size,
    ...
    def build_model(self, include_top=False, input_layer=None):
        ...
        Args:
            input_layer: keras layer
                * if None, then defaults to a regular input layer based on the
                shape
            extractor: boolean on whether or not to use the U-Net as a feature
                extractor or not
        """
        ...
        while level < self.max_pools:
            if level == 0:
                skip, pool = self.context_mod(input_layer,
                                              self.filter_list[level],
                                              pool_size=self.pool_list[0])
            elif level > 0:
                skip, pool = self.context_mod(pool, self.filter_list[level],
                                              pool_size=self.pool_list[level])
            skip_layers.append(skip)
            level += 1
        # No downsampling;  level at (depth) after the loop
        convs_bottom = self.context_mod(pool, self.filter_list[level],
                                        pool_size=None)
        ...
        while level > 0: # (** level = depth - 1 at the start of the loop)
            ...
            if level == self.max_pools:
                upsamp = self.localization_mod(convs_bottom,
                                               skip_layers[current_depth],
                                               self.filter_list[current_depth],
                                               upsampling_size=self.pool_list[current_depth])
            elif not level == self.max_pools:
                upsamp = self.localization_mod(upsamp,
                                               skip_layers[current_depth],
                                               self.filter_list[current_depth],
                                               upsampling_size=self.pool_list[current_depth])
            level -= 1
...
def SimpleUNet(include_top=False, input_layer=None, input_shape=(None, None, None),
               starting_filters=32, depth=4, n_convs=2, activation='relu',
               padding='same'):
    """
    ...
    context_mod = partial(model_utils.context_module_2D, n_convs=n_convs,
                          activation=activation)
    localization_mod = partial(model_utils.localization_module_2D,
                               n_convs=n_convs, activation=activation,
    ...
    while level < max_pools:
        ...
        level += 1
    # No downsampling;  level at (depth) after the loop
    convs_bottom = context_mod(pool, filter_list[level], pool_size=None)
    convs_bottom = context_mod(convs_bottom, filter_list[level], pool_size=None) # happens twice
    # localization pathway (upsampling with concatenation) [level (depth - 1) to level 1]
    while level > 0: # (** level = depth - 1 at the start of the loop)
        current_depth = level - 1
        if level == max_pools:
            upsamp = localization_mod(convs_bottom, skip_layers[current_depth],
                                      filter_list[current_depth],
                                      upsampling_size=pool_size)
        elif not level == max_pools:
            upsamp = localization_mod(upsamp, skip_layers[current_depth],
                                      filter_list[current_depth],
                                      upsampling_size=pool_size)
        level -= 1
    conv_transition = Conv2D(starting_filters, (1, 1),
                             activation=activation)(upsamp)
    # return feature maps
    if not include_top:
...
class U_CapsNet(object):
    """
    The U-CapsNet architecture is made up of a U-Net feature extractor for a
    capsule network.
    Attributes:
        ...
        decoder: whether or not you want to include a reconstruction decoder
            in the architecture
    """
    ...
    def build_model(self, model_layer=None, capsnet_type='r3',
        ...
        if model_layer is None:
            adap = AdaptiveUNet(2, self.input_shape, n_classes=self.n_class-1,
                                max_pools=6, starting_filters=5)
            model = adap.build_model(include_top=False, input_layer=x)
            # tensor_inp = model.output
        elif model_layer == "simple":
            model = SimpleUNet(include_top=False, input_layer=x,
        ...
        if capsnet_type.lower() == 'r3':
            train_model, eval_model = CapsNetR3(self.input_shape, n_class=2,
                                                decoder=self.decoder,
                                                add_noise=False,
                                                input_layer=model,
                                                upsamp_type=upsamp_type)
        elif capsnet_type.lower() == 'basic':
            train_model, eval_model = CapsNetBasic(self.input_shape, n_class=2,
                                                   decoder=self.decoder,
                                                   add_noise=False,
                                                   input_layer=model)
