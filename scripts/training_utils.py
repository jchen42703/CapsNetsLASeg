...
from capsnets_laseg.models.unet_models import U_CapsNet, CapsNetR3, AdaptiveUNet, SimpleUNet
from capsnets_laseg.models.metrics import dice_coefficient_loss, dice_hard, dice_plus_xent_loss
...
def get_generators(list_IDs, data_dirs, batch_size = 2, n_pos = 1, transform = None, max_patient_shape = (256, 320), steps = 1536, pos_mask = True):
    """
    ...
    gen = Transformed2DGenerator(train_ids, data_dirs, batch_size = batch_size, n_pos = n_pos, transform = transform, \
                                 max_patient_shape = max_patient_shape, steps_per_epoch = steps, pos_mask = pos_mask)
    gen_val = Transformed2DGenerator(val_ids, data_dirs, batch_size = batch_size, n_pos = n_pos, transform = transform, \
                                 max_patient_shape = max_patient_shape, steps_per_epoch = int(steps//6), pos_mask = pos_mask, shuffle = False)
    print("Steps per epoch: ", len(gen), "\nValidation Steps: ", len(gen_val))
    return gen, gen_val

def get_model(model_name, lr = 3e-5, input_shape = (256, 320, 1), decoder = False, inference = False):
    """
    Creates and compiles a specified model.
    Args:
        model_name (str): either "cnn", "ucapsr3", "capsr3", or "cnn-simple"
        lr (float): Learning rate. Defaults to 3e-5. Suggested learning rates:  CNNs: lr = 5e-4, Capsule Networks: lr = 3e-5
    Returns:
    ...
    if decoder:
        recon_wei = 0.2
        loss, loss_weighting = {"out_seg": dice_coefficient_loss, "out_recon": "mse"}, {"out_seg": 1., "out_recon": recon_wei}
    ...
    if model_name == "ucapsr3":
        ...
        train_model, eval_model = ucaps.build_model(model_layer = "simple", capsnet_type = "r3", upsamp_type = "subpix")
    elif model_name == "capsr3":
        train_model, eval_model = CapsNetR3(input_shape, n_class = 2, decoder = decoder, upsamp_type = "subpix")
    elif model_name == "cnn":
        adap = AdaptiveUNet(2, input_shape, n_classes = 1, max_pools = 6, starting_filters = 15)
    ...
    train_model.compile(optimizer = opt, loss = loss, metrics = metrics, loss_weights = loss_weighting)
    if inference:
...
def get_callbacks(model_name, checkpoint_dir = "/content/checkpoint.h5", decoder = False):
    ...
    ckpoint = ModelCheckpoint(checkpoint_dir, monitor = monitor, save_best_only = True, period = 3)
    stop = EarlyStopping(monitor = monitor, min_delta = min_delta, patience = patience_stop, mode = mode, restore_best_weights = True)
