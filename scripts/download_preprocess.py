...
def download_dataset(dset_path, dataset,
    ...
    """
    tar_path = join(dset_path, f"{dataset}.tar")
    gdd.download_file_from_google_drive(file_id=id,
                                        dest_path=tar_path, overwrite=False,
                                        unzip=False)

    if not os.path.exists(join(dset_path, dataset)):
...
if __name__ == "__main__":
    ...
    input_dir = join(args.dset_path, "Task02_Heart")
    output_dir = join(args.output_path, "Preprocessed_Heart")
    ...
    for filename in glob.glob(join(input_dir, "imagesTr", "._*")):
        os.remove(filename)
    for filename in glob.glob(join(input_dir, "labelsTr", "._*")):
        os.remove(filename)
